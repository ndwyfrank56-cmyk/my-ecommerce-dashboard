...
def auth_google():
    ...
    try:
        ...
        flash('Error initiating Google authentication. Please try again.', 'error')
        return redirect('/login')
...
def auth_google_callback():
    ...
    try:
        ...
        try:
            ...
            # Check if worker exists with this email
            cur.execute("""
                SELECT wl.login_id, wl.worker_id, wl.username, w.name, w.email, w.profession, w.deptName
                FROM worker_login wl
                JOIN workers w ON wl.worker_id = w.worker_id
                WHERE w.email = %s
            """, (email,))
            ...
            if user:
                ...
                if allowed_pages:
                    return redirect(f'/{allowed_pages[0]}')
                else:
                    return redirect('/access-denied')
            else:
                # New user - create account if auto-create is enabled
                cur.execute("""
                    INSERT INTO workers (name, email, phone, profession, deptName, created_at)
                    VALUES (%s, %s, %s, %s, %s, NOW())
                """, (name, email, '', 'worker', 'General'))
                
                new_worker_id = cur.lastrowid
                
                # Create login entry with a temporary password
                temp_password = secrets.token_urlsafe(16)
                hashed_password = generate_password_hash(temp_password)
                
                cur.execute("""
                    INSERT INTO worker_login (worker_id, username, password)
                    VALUES (%s, %s, %s)
                """, (new_worker_id, email, hashed_password))
                
                # Grant default pages
                default_pages = ['products', 'orders', 'customers']
                ...
                session['name'] = name
                session['email'] = email
                session['profession'] = 'worker'
                session['department'] = 'General'
                session['allowed_pages'] = default_pages
                session['auth_method'] = 'google'
            ...
            print(f"Database error during Google auth: {e}")
            flash('Error processing authentication. Please try again.', 'error')
            return redirect('/login')
